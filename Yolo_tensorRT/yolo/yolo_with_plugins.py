...
try:
    import os
    dll_path = os.path.join(os.path.dirname(__file__), '../plugins/libyolo_layer.so')    
...
class TrtYOLO(object):
    ...
    def __init__(self, model, category_num=80, batch_size=1, cuda_ctx=None):
    ...
    def get_batches(self, img_list, letter_box):        
        batches= [_preprocess_yolo(one_img, self.input_shape, letter_box = letter_box) for one_img in img_list]
        batch_datas= np.stack(batches)
        return batch_datas

    def batch_detect(self, img_list, conf_th=0.3, letter_box=None):
        batch_datas= self.get_batches(img_list,letter_box)    
        # Set host input to the image. The do_inference() function
        # will copy the input to the GPU before executing.
        self.inputs[0].host = np.ascontiguousarray(batch_datas)
        if self.cuda_ctx:
            self.cuda_ctx.push()
        ...
        for idx in range(len(trt_outputs)):
            trt_outputs[idx]= trt_outputs[idx].reshape(self.batch_size, -1)

        batch_boxes, batch_scores, batch_classes= [], [], []
        for batch_idx, img in enumerate(img_list):
            one_img_trt_outputs= [trt_outputs[i][batch_idx] for i in range(len(trt_outputs))]
            
            boxes, scores, classes = _postprocess_yolo(
                one_img_trt_outputs, img.shape[1], img.shape[0], conf_th,
